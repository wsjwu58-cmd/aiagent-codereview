...
*/
export { ThinkingPanel } from './ThinkingPanelView'
  ...
  return (
    <div className="thinking-panel">
      <div className="thinking-panel__header">
        <Space size={8}>
          <Typography.Text strong>思考过程</Typography.Text>
          <Tag color={status === 'thinking' ? 'processing' : status === 'error' ? 'error' : 'success'}>
            {status === 'thinking' ? '分析中' : status === 'error' ? '失败' : '已完成'}
          </Tag>
        </Space>
          ...
          <div key={`${step.step}-${step.agentId}-${step.type}`} className="thinking-step">
            ...
            {step.type === 'TOOL_CALL' ? (
              <div className="thinking-step__content">
                <Typography.Text strong>调用工具：{step.tool}</Typography.Text>
