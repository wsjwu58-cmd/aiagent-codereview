...
export type KnowledgeRecordType = 'REVIEW_HISTORY' | 'PDF_NORM' | 'CHAT_HISTORY'

export interface KnowledgeManageRecord {
  id: string
...
export interface KnowledgeManagePageResponse {
  content: KnowledgeManageRecord[]
...
export interface KnowledgeDeleteItem {
  id: string
  type: KnowledgeRecordType
}
...
export interface LocalCodeAnalyzeRequest {
  folderPath: string
  fileFilters?: string[]
  autoWrite?: boolean
}
...
export async function downloadReviewPdf(reviewId: string) {
  const res = await client.get(`/review/${reviewId}/pdf`, {
    responseType: 'blob',
  })
...
export async function getKnowledgeRecords(params: {
  type?: KnowledgeRecordType
  projectId?: string
  keyword?: string
  page?: number
  ...
  const suffix = query.toString()
  const url = suffix ? `/knowledge/records?${suffix}` : '/knowledge/records'
  const res = await client.get<ApiResponse<KnowledgeManagePageResponse>>(url)
  return res.data.data
}

export async function deleteKnowledgeRecord(id: string, type?: KnowledgeRecordType) {
  const suffix = type ? `?type=${encodeURIComponent(type)}` : ''
  await client.delete(`/knowledge/records/${encodeURIComponent(id)}${suffix}`)
}

export async function batchDeleteKnowledgeRecords(records: KnowledgeDeleteItem[]) {
  await client.post('/knowledge/records/batch', { records })
}
...
export async function streamLocalCodeAnalysis(
  payload: LocalCodeAnalyzeRequest,
  onEvent: (payload: StreamEventPayload) => void,
  signal?: AbortSignal,
) {
  ...
  if (!response.ok || !response.body) {
    throw new Error(`本地代码分析请求失败: ${response.status}`)
  }
