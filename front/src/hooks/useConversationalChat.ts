...
*/
export { useConversationalChat } from './useConversationalChatClean'
export type { ConversationMessage } from './useConversationalChatClean'
...
export function useConversationalChat({
  ...
  const sendDeepAnalysis = async () => {
    ...
    setMessages((prev) => [
      ...prev,
      { id: userId, role: 'user', content: question, timestamp: Date.now() },
      { id: analysisId, role: 'assistant', content: '', timestamp: Date.now(), thinkingSteps: [], analysisStatus: 'thinking' },
    ])
